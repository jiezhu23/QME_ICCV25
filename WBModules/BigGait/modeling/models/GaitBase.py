...
class Pre_ResNet9(ResNet):
    def __init__(self, type, block, channels=[32, 64, 128, 256], in_channel=1, layers=[1, 2, 2, 1],
                 strides=[1, 2, 2, 1], maxpool=True):
...
from ...utils.config import get_valid_args, is_dict
from ...utils.common import get_attr_from, is_list
...
class Baseline(nn.Module):
    def __init__(self, model_cfg):
        ...
        self.HPP = SetBlockWrapper(self.HPP)
    ...
    def pad_width(self, images, width=6):
        # [n, c, s, h, w]
        # return F.pad(images, (width, width), "constant", 0)
        return images

    def vis_forward(self, part, rgb, seqL):
        part = self.pre_part(self.pad_width(part))  # [n, c, s, h, w]
        rgb = self.pre_rgb(self.pad_width(rgb))  # [n, c, s, h, w]
        outs = self.fusion([part, rgb])
        return part, rgb, outs

    def forward(self, part, rgb, seqL):
        part = self.pre_part(self.pad_width(part))  # [n, c, s, h, w]
        rgb = self.pre_rgb(self.pad_width(rgb))  # [n, c, s, h, w]
        outs = self.fusion([part, rgb])
        del part, rgb
        ...
        # heat_mapt = rearrange(outs, 'n c h w -> n h w c')[:,None,...]

        # Horizontal Pooling Matching, HPM
        outs = self.HPP(outs)  # [n, c, s, p]
