...
from ..utils.reidtools import calc_splits
...
class GSTA(nn.Module):
    ...
    def forward(self, x, adj, *args):
        ...
        for idx, n in enumerate(self.total_split_list):
            v_g.append(self.parts_avgpool[idx](f).view(B, S, c, n))
        v_g = torch.cat(v_g, dim=3)
        f = v_g.transpose(2, 3).contiguous().view(B, S * self.total_split, c)
        # graph propagation
...
def gsta(num_classes, loss, last_stride, num_split, num_gb, num_scale, pyramid_part, use_pose, learn_graph,
         pretrained=True, consistent_loss=False, **kwargs):
    model = GSTA(
