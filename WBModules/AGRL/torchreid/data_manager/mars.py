...
class Mars(object):
    def __init__(self, root='data', min_seq_len=0, verbose=True, **kwargs):
        ...
        track_train = np.loadtxt(self.track_train_info_path).astype(int)
        track_test = np.loadtxt(self.track_test_info_path).astype(int) 
        query_IDX = np.loadtxt(self.query_IDX_path).astype(int) 
