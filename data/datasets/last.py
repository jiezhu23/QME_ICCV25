...
class LaST(object):
    ...
    def get_pid2label_and_clothes2label(self, dir_path):
        if hasattr(self, 'body_data'):
            img_paths = [osp.join(dir_path, idx) for idx in self.body_data[dir_path].keys()] # h5 file format
        else:
    ...
    def _check_before_run(self):
        ...
        if osp.exists(osp.join(self.root, 'last.h5')):
            self.body_data = h5py.File(osp.join(self.root, 'last.h5'), 'r')
            print('h5 format file detected, using h5 format data')
            if not osp.exists(osp.join(self.root, 'last_face.h5')):
                print('face data is not available, using body data only')
            else:
                self.face_data = h5py.File(osp.join(self.root, 'last_face.h5'), 'r')
